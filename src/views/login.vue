...
<script>
import {setStore} from '../until/until.js'
export default {
    name:"login",
    data(){
        return{
            ...
            pwd:'',
            code:'',
            codeUrl:'/act/getCode?time='+new Date().getTime()
        }
    },
    methods:{
        async loginBtn(){
            ...
            await this.$store.dispatch("LoginModule/POST_LOGIN",{username:_this.name,password:_this.pwd,authCode:_this.code});
            ...
            let userData={
                token:data.out.token,
                username: data.out.account.accountName
            }
            setStore('userInfo',userData)
            _this.$router.push("/home");
        },
        imgClick(){
            this.codeUrl ='/act/getCode?time='+new Date().getTime()
        }
    },
    created(){
    },
