  ...
  <div class="simple-code-page">
    ...
    <div class="simple-code-cont">
          ...
          <el-table :data="tableDataLeft" stripe style="width: 100%">
            ...
            <el-table-column fixed="right" label="操作" width="180">
              <template slot-scope="scope">
                <el-button size="mini" :type="scope.row.state!=='00001002'?'danger':'' " @click="codeTypeStatusBtn(scope.row)">{{scope.row.state | statusBtnTxt}}</el-button>
                <el-button size="mini" @click="codeTypeEdiBtn(scope.row)">编辑</el-button>
                <el-button class='mt10' size="mini" @click="viewBtn(scope.row)">查看</el-button>
              </template>
            </el-table-column>
          </el-table>
          <myPackage
      ...
      <div class="simple-code-right">
        ...
        <div v-if='tableDataRight.length>0' class="simple-code-table">
          <el-table :data="tableDataRight" stripe style="width: 100%">
            <el-table-column prop="codeTypeId" label="属性编码"></el-table-column>

            <el-table-column prop="codeValueName" label="属性名称"></el-table-column>
            ...
            <el-table-column fixed="right" label="操作" width="180">
              <template slot-scope="scope">
                ...
                <el-button  size="mini" :type="scope.row.state!=='00001002'?'danger':'' " @click="attrChangeBtn(scope.row)">{{scope.row.state | statusBtnTxt}}</el-button>
                <el-button  size="mini" @click="ediAttrBtn(scope.row)">编辑</el-button>
              </template>
            </el-table-column>
          </el-table>
          <myPackage
    ...
    <!-- 添加属性 -->
     ...
     <div>
        <el-row :gutter="10" >
          ...
          <el-col :span="12"><el-input :disabled='addInfoData.type!="save"' v-model='addInfoData.codeValueId' maxlength="8"  show-word-limit clearable placeholder="请输入内容"></el-input></el-col>
        </el-row>
         <el-row :gutter="10" class="mt20">
          <el-col :span="4"><span class="my-span-notice">*</span>属性名称:</el-col>
          <el-col :span="12"><el-input v-model='addInfoData.codeValueName' maxlength="30"  show-word-limit clearable placeholder="请输入内容"></el-input></el-col>
        </el-row>
     </div>
      <span slot="footer" class="dialog-footer">
...
export default {
  ...
  data() {
    return {
      packTotalLeft: 1,
      packTotalRight: 1,
      brea: [{ name: "系统管理", url: "" }, { name: "简单码表", url: "" }],
      tableDataLeft:[],
      tableDataRight:[],
      typeSeach:{
        pageNo:1,
  ...
  methods: {
    async CodeTypeList(){
      await this.$store.dispatch("SystemModule/POST_CODE_TYPE_LIST",this.typeSeach)
      let data = this.$store.state.SystemModule.POST_CODE_TYPE_LIST;
      this.tableDataLeft = data.out.list;
      this.packTotalLeft = parseInt(data.out.totalSize)
    },
    async addCodeType(addTypeData){
      await this.$store.dispatch("SystemModule/POST_ADD_CODE_TYPE",addTypeData)
      let data = this.$store.state.SystemModule.POST_ADD_CODE_TYPE;
      this.CodeTypeList()
    },
    async codeChangeStatus(typeData){
      await this.$store.dispatch("SystemModule/GET_CODE_TYPE_STATUS",typeData)
      let data = this.$store.state.SystemModule.GET_CODE_TYPE_STATUS;
      this.$message.success("操作成功")
      this.CodeTypeList()
    },
    async CodeValueList(){
      await this.$store.dispatch("SystemModule/POST_CODE_INFO_LIST",this.infoSeach)
      let data = this.$store.state.SystemModule.POST_CODE_INFO_LIST;
      this.tableDataRight = data.out.list;
      this.packTotalRight = parseInt(data.out.totalSize)
    },
    async addCodeInfo(addInfoData){
      await this.$store.dispatch("SystemModule/POST_ADD_CODE_INFO",addInfoData)
      let data = this.$store.state.SystemModule.POST_ADD_CODE_INFO;
      this.CodeValueList()
    },
    async codeInfoAttrChange(infoData){
      await this.$store.dispatch("SystemModule/POST_CODE_ATTR_STATUS",infoData)
      let data = this.$store.state.SystemModule.POST_CODE_ATTR_STATUS;
      this.CodeValueList()
    },
    ...
    addValSure(){
      if(!this.addInfoData.codeValueId){
          this.$message.warning("请填写属性编码")
        return
      }
      if(!this.addInfoData.codeValueName){
    ...
    attrChangeBtn(row){
      let _this = this;
      myConfirm(_this,'是否进行此操作？',function(){
        _this.codeInfoAttrChange({codeInfoValue:row.codeValueId})
      })
    }
  },
  created() {
