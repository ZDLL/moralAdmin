         ...
         <el-form :inline="true" class="demo-form-inline">
            ...
            <el-form-item>
               <el-button type="primary" plain @click="searchBtn">查询</el-button>
                <el-button type="primary" @click="addParaBtn">添加参数</el-button>      
            </el-form-item>
         </el-form>
        <el-divider content-position="left">参数列表</el-divider>
        <div v-if='tableData.length>0' class="page-table">
          <el-table :data="tableData" stripe style="width: 100%">
            ...
            <el-table-column fixed="right" label="操作" width="180">
              <template slot-scope="scope">
                <el-button :type="scope.row.state!=='00001002'?'danger':'' " size="mini" @click="stateChange(scope.row)">{{scope.row.state | statusBtnTxt}}</el-button>
                <el-button size="mini" @click='editParaBtn(scope.row)'>编辑</el-button>
              </template>
            </el-table-column>
          </el-table>
          <myPackage
...
export default {
  ...
  data() {
    return {
      brea: [{ name: "系统管理", url: "" }, { name: "系统参数", url: "" }],
      tableData: [],
      packTotal: 1,
      paraDia:false,
      paraData:{
  ...
  methods: {
    async getParaList(){
        ...
        "SystemModule/POST_PARA_CODE_LISY",
        this.searchData
        );
        let data = this.$store.state.SystemModule.POST_PARA_CODE_LISY;
    ...
    async paraSave(saveData){
        await this.$store.dispatch(
        "SystemModule/POST_PARA_CODE_SAVE",
        saveData
      );
      let data = this.$store.state.SystemModule.POST_PARA_CODE_SAVE;
